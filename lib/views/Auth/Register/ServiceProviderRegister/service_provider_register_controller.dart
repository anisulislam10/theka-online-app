...
// Replace the entire signInWithGoogle method with this:
      ...
      await showDialog(
        ...
        builder: (_) => SavingProgressWidget(
          task: (updateProgress) async {
            // NOTE: We intentionally do NOT update the Firebase Auth password here.
            // The dummy password (pw_PHONE_stable) must remain unchanged so that
            // future logins via phone or email can use it consistently.
            // The user's real password is stored as a passwordHash in Firestore
            // for application-level email login verification.
            debugPrint("ℹ️ Skipping Firebase Auth password update — using Firestore passwordHash instead");
  ...
  Future<void> continueToDocumentsWithoutOtp(BuildContext context) async {
    ...
    String rawPhone = phoneController.text.trim().replaceAll(RegExp(r'\D'), '');
    ...
    try {
      ...
      if (e.code == 'email-already-in-use') {
        ...
        try {
          ...
          if (signInError is FirebaseAuthException && (signInError.code == 'invalid-credential' || signInError.code == 'wrong-password')) {
            try {
              print("⚠️ Dummy password mismatch. Trying real entered password...");
              UserCredential userCredential = await _auth.signInWithEmailAndPassword(
                email: dummyEmail,
                password: passwordController.text.trim(),
              );
              await userCredential.user?.updatePassword(dummyPassword);
              print("✅ Password recovered and reset to dummy for consistency");
              ...
              _showErrorDialog(context, "Auth Error: This phone number has a previous partial registration with a different password. Please try registering again with the password you previously entered, or login instead.");
            }
          } else {
